...
@section('content')
    <div class="title">
        <h1 class="h3 mb-0 text-gray-800">Master | <span class="text-secondary fs-5">Data Galeri</span></h1>
    </div>
                            ...
                            <tbody>
                                @foreach ($galeri as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <img src="{{ asset('storage/galeri/' . $item->file_foto) }}" alt="Foto"
                                                style="width: 50px; height: auto;">
                                        </td>
                                        <td>{{ ucwords($item->jenis_foto) }}</td>
                                        <td >
                                            @if ($item->is_featured)
...
@push('scripts')
    ...
    <script>
        function create() {
            $.get("{{ route('galeri.create') }}", {}, function(data, status) {
                $("#staticBackdropLabel").html('Tambah Galeri');
                $("#page").html(data);
        ...
        function edit(id) {
            $.get("{{ url('master/galeri') }}/" + id + "/edit", {}, function(data, status) {
                $("#staticBackdropLabel").html('Edit Galeri');
                $("#page").html(data);
                $("#staticBackdrop").modal("show");
            });
        }
