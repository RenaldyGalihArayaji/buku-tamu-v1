...
@section('content')
    <div class="title">
        <h1 class="h3 mb-0 text-gray-800">Master | <span class="text-secondary fs-5">Data Tamu</span></h1>
    </div>
...
@push('scripts')
    ...
    <script>
        function create() {
            $.get("{{ route('tamu.create') }}", {}, function(data, status) {
                $("#staticBackdropLabel").html('Tambah Tamu');
                $("#page").html(data);
        ...
        function store() {
            ...
            var dataToForm = $("#form-tamu").serialize(); //Mengambil data di dalam form
        ...
        function edit(id) {
            $.get("{{ url('tamu') }}/" + id + "/edit", {}, function(data, status) {
                $("#staticBackdropLabel").html('Edit Tamu');
                $("#page").html(data);
                $("#staticBackdrop").modal("show");
            });
        }
        ...
        function show(id) {
            $.get("{{ url('tamu') }}/" + id, {}, function(data, status) {
                $("#staticBackdropLabel").html('Detail Tamu');
                $("#page").html(data);
                $("#staticBackdrop").modal("show");
            });
        }
