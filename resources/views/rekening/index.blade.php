...
@section('content')
    <div class="title">
        <h1 class="h3 mb-0 text-gray-800">Master | <span class="text-secondary fs-5">Data Rekening</span></h1>
    </div>
                            ...
                            <tbody>
                                @foreach ($rekening as $item)
                                    <tr>
                                        ...
                                        <td>{{ ucwords($item->nama_bank) }}</td>
                                        <td>{{ ucwords($item->nomor_rekening) }}</td>
                                        <td>{{ ucwords($item->nama_pemilik_rekening) }}</td>
                                        <td>
                                            @if ($item->is_active)
                                                <span class="badge bg-success"><i class="ti-check"></i></span>
                                            @else
                                                <span class="badge bg-danger"><i class="ti-close"></i></span>
                                            @endif
                                        </td>
                                        <td>
...
@push('scripts')
    ...
    <script>
        function create() {
            $.get("{{ route('rekening.create') }}", {}, function(data, status) {
                $("#staticBackdropLabel").html('Tambah Rekening');
                $("#page").html(data);
        ...
        function store() {
            ...
            var dataToForm = $("#form-rekening").serialize(); //Mengambil data di dalam form
        ...
        function edit(id) {
            $.get("{{ url('master/rekening') }}/" + id + "/edit", {}, function(data, status) {
                $("#staticBackdropLabel").html('Edit Rekening');
                $("#page").html(data);
                $("#staticBackdrop").modal("show");
            });
        }
